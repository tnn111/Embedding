#!/usr/bin/env python3
"""
Variational Autoencoder for multi-scale k-mer frequency distributions (no 1-mers).

Input: 6-mer through 2-mer frequencies (2,770 canonical k-mers), CLR-transformed.
       1-mers (AT/GC) are excluded to remove GC content from the embedding.
       ...
       - Columns 2771-2772: 1-mers (SKIPPED)

Output: Reconstructed CLR-transformed frequencies (2,770 features).
Loss: MSE (mean squared error) for reconstruction.

Fully-connected architecture with 384-dimensional latent space.
Uses the same input files as VAE.py but ignores the last 2 columns.
"""
...
# Global constants
INPUT_DIM = 2770  # 6-mer through 2-mer canonical frequencies (2080+512+136+32+10), no 1-mers
LATENT_DIM = 384
SEED = 42

# Column ranges in k-mers.npy (0-indexed, col 0 is sequence length)
COL_START = 1     # Start of 6-mers (after length column)
COL_END = 2771    # End of 2-mers (exclusive), skipping 1-mers at cols 2771-2772
...
class VAE(Model):
    ...
    def _build_encoder(self):
        ...
        Architecture:
            Input (2770,) → Dense(1024) → Dense(512) → z_mean(384), z_log_var(384)
        """
    ...
    def _build_decoder(self):
        ...
        Architecture:
            Input (384) → Dense(512) → Dense(1024) → Dense(2770)
        """
...
def load_data_to_memory(file_path: str, start_idx: int, end_idx: int) -> np.ndarray:
    """Load 6-mer through 2-mer data into memory as float32 with CLR transform (no 1-mers).
    ...
    # Load columns for 6-mers through 2-mers (columns 1-2770, skipping length and 1-mers)
...
def main():
    parser = argparse.ArgumentParser(description = 'Train VAE on 6-mer through 2-mer frequency data (no 1-mers/GC)')
