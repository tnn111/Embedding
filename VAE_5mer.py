#!/usr/bin/env python3
"""
Variational Autoencoder for 5-mer through 1-mer frequency distributions.

Drops 6-mers entirely, using only 5-mer through 1-mer canonical k-mer
frequencies (692 features). 5-mers are more robust to assembly errors
(5 affected per substitution vs 6) and have better statistical coverage
at short contig lengths (~10 counts/bin at 5 kbp vs ~2.4 for 6-mers).

Input columns from the standard k-mer .npy format:
    - Column 0: sequence length (skipped)
    - Columns 1-2080: 6-mers (SKIPPED)
...
Output: Reconstructed CLR-transformed frequencies (692-dim).
Loss: MSE for reconstruction + beta * KL divergence.

Architecture: 692 -> 512 -> 256 -> 128 (latent). ~1.1M parameters.
"""
...
# Global constants
INPUT_DIM = 692   # 5-mer through 1-mer canonical frequencies (512+136+32+10+2)
LATENT_DIM = 128
SEED = 42

# Column ranges in k-mers.npy (0-indexed, col 0 is sequence length)
COL_START = 2081  # Start of 5-mers (skip length column and 6-mers)
COL_END = 2773    # End of 1-mers (exclusive)

# K-mer sizes within the INPUT_DIM features (local indices, 0-based)
KMER_SIZES = {
    '5mer': (0, 512),       # 512 features
    '4mer': (512, 648),     # 136 features
    '3mer': (648, 680),     # 32 features
    '2mer': (680, 690),     # 10 features
    '1mer': (690, 692),     # 2 features
}
...
class KLWarmupCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs = None):
        # Reset dependent callbacks once when warmup ends
...
class VAECheckpoint(keras.callbacks.Callback):
    ...
    def __init__(self, filepath_prefix = 'vae', monitor = 'val_loss', verbose = 0,
                 initial_best = None):
        super().__init__()
...
class VAE(Model):
    ...
    def _build_encoder(self):
        ...
        Architecture:
            Input (692,) -> Dense(512) -> Dense(256) -> z_mean(128), z_log_var(128)
        """
    ...
    def _build_decoder(self):
        ...
        Architecture:
            Input (128) -> Dense(256) -> Dense(512) -> Dense(692)
        """
...
def clr_transform_inplace(data: np.ndarray) -> None:
    ...
    Uses a Jeffreys prior pseudocount of 0.5/n_features per group.
...
def load_data_to_memory(file_path: str, start_idx: int, end_idx: int) -> np.ndarray:
    """Load 5-mer through 1-mer data into memory as float32 with CLR transform.
    ...
    # Load columns for 5-mers through 1-mers (skip length and 6-mers)
...
def main():
    parser = argparse.ArgumentParser(description = 'Train VAE on 5-mer through 1-mer frequency data')
